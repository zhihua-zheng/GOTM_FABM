...
of wind, wind speed, and wind direction. High resolution
files may also include wind gust, and both scalar and 
vector wind speeds.
...
For some deployments, the only available kind of wind speed
is vector wind speed (VSPD), that is computed from the U and V
components. For other deployments, the only available speed
is scalar wind speed (SSPD), which directly measured using 
different instruments. For 5-day, monthly, and quarterly averaging 
intervals only vector wind speed is included, and this is just 
called WSPD.

Instrument height is indicated in the data files, with height 
...
In netcdf files you will find quality codes with the same 
shape as the data, and with variable names and attributes 
which give details on which time series the qualities are 
associated with, e.g.., speed and direction.
