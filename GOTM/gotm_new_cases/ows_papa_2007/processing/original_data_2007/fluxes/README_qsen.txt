...
including sensible heat flux.

Fluxes are computed using the warm-layer and cool-skin corrections 
which are built into the COARE 3.0b algorithm. To provide the best 
possible estimates of the high resolution surface fluxes we compute 
them only at times when there is a complete set of input data.

Wind speeds input to the COARE algorithm are adjusted for 
surface currents using available shallow current data, and
if not available, using the OSCAR 1/3 degree currents 
located here

  http://podaac-ftp.jpl.nasa.gov/dataset/OSCAR_L4_OC_third-deg

OSCAR currents are interpolated to the long-term average
buoy locations, and to the data times. Surface currents
components are simply subtracted from the wind components, 
and the wind speed is computed from the these adjusted
winds components.

Available surface barometric pressure data are input to the 
COARE algorithm and at times when pressures are not available, 
we use the mean of all available data at the buoy site.
