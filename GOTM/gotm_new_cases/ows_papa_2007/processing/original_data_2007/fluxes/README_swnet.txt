...
Net shortwave radiation (W m^-2) is computed using the 
following equation, 

 swnet = sw *(1.0 - albedo / 100.0)

where sw is the observed incident shortwave radiation, 
and albedo is interpolated from the ISCCP satellite
climatology located at  

  http://isccp.giss.nasa.gov/products/browsefmt.html

This climatology is interpolated to the long-term average 
buoy location and the data times, to be used in the above
equation.
