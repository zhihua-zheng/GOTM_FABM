...
out.tx = double(squeeze(ncread(fname,'tx'))); % wind stress (tau_x/rho) [m^2/s^2]
out.ty = double(squeeze(ncread(fname,'ty'))); % wind stress (tau_y/rho) [m^2/s^2]
out.u_taus = double(squeeze(ncread(fname,'u_taus'))); % water-side surface friction velocity [m/s]
